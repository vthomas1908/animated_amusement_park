...
//	glEnable(GL_CULL_FACE);
	...
	wheel.Initialize();
	bridge.Initialize();
	ticket_booth.Initialize();
	tree1.Initialize(); tree2.Initialize(); tree3.Initialize();
	tree4.Initialize(); tree5.Initialize(); tree6.Initialize();
	merry.Initialize();
	soda.Initialize();
	ice_cream.Initialize();
	sign.Initialize();
	rocket.Initialize(1, .15);
	rocket2.Initialize(.15, 1);
	for (int i = 0; i < 75; i++)
		walk[i].Initialize();
	for (int i = 0; i < 6; i++)
		tree[i].Initialize();
...
*/
	...
	glTranslated(0, 0, 4);
	glColor3f(1, 1, 1);
	gluCylinder(gluNewQuadric(), .25, .25, 1.25, 5, 5);
	
	glTranslated(0, 0, 1);
	glRotated(10, 0, 1, 0);
	glColor3f((float)15/(float)256, (float)116/(float)256, (float)247/(float)256);
	gluCylinder(gluNewQuadric(), 1, 2, 3, 10, 10);
	glColor3f(1, 0, 0);
	gluDisk(gluNewQuadric(), 0, 1, 10, 10);
	glRotated(10, 0, 1, 0);
	glColor3f(1, 1, 1);
	gluCylinder(gluNewQuadric(), .25, .25, 4, 5, 5);
	glPopMatrix();
	...
	gluCylinder(gluNewQuadric(), 0, 2, 3, 10, 10);
	glTranslated(0, 0, 3);
	glColor3f((float)242/(float)256, (float)222/(float)256, (float)208/(float)256);
	...
	glPushMatrix();
	tree1.Draw(2); 
	glTranslatef(-35, -15, 0); tree2.Draw(1.5); 
	glTranslatef(15, -10, 0); tree3.Draw(3); 
	glTranslatef(18, 4, 0); tree4.Draw(1); 
	glTranslatef(32, 5, 0); tree5.Draw(1); 
	glTranslatef(-5, 30, 0); tree6.Draw(1);
	glTranslatef(-40, 4, 0); tree[0].Draw(1);
	glTranslatef(-10, 11, 0); tree[1].Draw(1);
	glTranslatef(-10, -15, 0); tree[2].Draw(1);
	glTranslatef(25, -14, 0); tree[3].Draw(1);
	glTranslatef(15, 4, 0); tree[4].Draw(1);
	glTranslatef(10, -20, 0); tree[0].Draw(1);
	glPopMatrix();
}
