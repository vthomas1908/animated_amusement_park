...
Walk::Initialize(void)
{
    ...
    if ( ! ( image_data = (ubyte*)tga_load("cobblestone.tga", &image_width,
					   &image_height, TGA_TRUECOLOR_24) ) )
    {
	fprintf(stderr, "TicketBooth::Initialize: Couldn't load cobblestone.tga\n");
	return false;
    }
	...
	glTexCoord2f(0, 1);  glVertex3f(-2, 2, .05); 
	glTexCoord2f(0, 0);  glVertex3f(-2, -2, .05); 
	glTexCoord2f(1, 0);  glVertex3f(2, -2, .05);
	glTexCoord2f(1, 1);  glVertex3f(2, 2, .05); 
...
bool Walk::Resize(ubyte *data, int *height, int *width)
{
